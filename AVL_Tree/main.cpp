...
/* virtual */ void FsLazyWindowApplication::Interval(void)
{
	...
	else if(FSKEY_V==key)
	{
		...
		tree.TreeToVine(mouseOn);
		needRedraw = true;
	}
	else if(FSKEY_T == key)
	{
		if(!tree.IsVine(mouseOn))
		{
			printf("This is not a Vine!!!\n");
		}
		else if(tree.IsVine(mouseOn))
		{
			int size = tree.GetN(mouseOn);
			printf("%i\n", size);
			tree.VineToTree(mouseOn, size);
			needRedraw = true;
		}
	}
	else if(FSKEY_P==key)
	{
		printf("%i\n", tree.GetHeight(mouseOn));
		needRedraw = true;
	}
